...
/* =======================================================
   KONEKSI DATABASE
======================================================= */
// (Bagian ConnectDB TETAP SAMA seperti sebelumnya)
func ConnectDB() {
	dbOnce.Do(func() {
		_ = godotenv.Load()
		...
		if secret == "" { secret = "MYTRAVEL_SECRET_KEY_GANTI_INI" }
...
// Handler : Ini adalah fungsi yang dicari oleh Vercel!
// Signature harus: func(w http.ResponseWriter, r *http.Request)
func Handler(w http.ResponseWriter, r *http.Request) {
	// Pastikan DB connect
	ConnectDB()
	
	// Panggil router utama kita
	NewRouter().ServeHTTP(w, r)
}

// NewRouter : Fungsi router utama (sebelumnya bernama Handler)
// Kita ubah namanya agar tidak bentrok dengan Handler milik Vercel
func NewRouter() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		/* -------- AUTH -------- */
		if p == "/api/auth/register" { Register(w, r); return }
		if p == "/api/auth/login" { Login(w, r); return }
		if p == "/api/auth/logout" { Logout(w, r); return }
		if p == "/api/auth/me" { Me(w, r); return }

		/* -------- PLACES -------- */
		if p == "/api/places" && r.Method == "GET" { GetPlaces(w, r); return }
		if p == "/api/places" && r.Method == "POST" { CreatePlace(w, r); return }
		...
		/* -------- MY PLACES -------- */
		if p == "/api/my-places" && r.Method == "GET" { GetMyPlaces(w, r); return }
		if strings.HasPrefix(p, "/api/my-places/") && r.Method == "DELETE" { DeletePlace(w, r); return }

		/* -------- REVIEWS -------- */
		if p == "/api/reviews" && r.Method == "GET" { GetReviews(w, r); return }
		if p == "/api/reviews" && r.Method == "POST" { CreateReview(w, r); return }
		if p == "/api/my-reviews" && r.Method == "GET" { GetMyReviews(w, r); return }
		if strings.HasPrefix(p, "/api/my-reviews/") && r.Method == "DELETE" { DeleteReview(w, r); return }
		if strings.HasPrefix(p, "/api/reviews/") && r.Method == "DELETE" { DeleteReview(w, r); return }

		/* -------- PHOTO -------- */
		if strings.HasPrefix(p, "/api/photo/") { GetPhoto(w, r); return }
...
/* =======================================================
   FUNGSI LOGIC (COPY SEMUA FUNGSI DI BAWAH INI TETAP SAMA)
======================================================= */
// (Silakan salin fungsi Register, Login, GetPlaces, dll dari kode sebelumnya.
//  Tidak ada perubahan logic di sana, hanya pastikan semua fungsi CRUD ada di sini)
