...
// EnableCORS: Fungsi ini WAJIB ADA agar tidak error 'undefined'
...
// Handler : Entry point untuk Vercel Serverless Function
...
// NewRouter : Router utama aplikasi
func NewRouter() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		
		// Panggil EnableCORS di sini
		EnableCORS(w, r)
...
func DeletePlace(w http.ResponseWriter, r *http.Request) {
	...
	id := strings.TrimPrefix(r.URL.Path, "/api/places/")
	id = strings.TrimPrefix(id, "/api/my-places/") // Handle kedua route
	objID, _ := primitive.ObjectIDFromHex(id)
...
func GetMyReviews(w http.ResponseWriter, r *http.Request) {
	...
	pipeline := bson.A{
		...
		bson.M{"$addFields": bson.M{"place_obj_id": bson.M{"$toObjectId": "$place_id"}}},
		bson.M{"$lookup": bson.M{"from": "Data_Travel", "localField": "place_obj_id", "foreignField": "_id", "as": "place_info"}},
		bson.M{"$unwind": bson.M{"path": "$place_info", "preserveNullAndEmptyArrays": true}},
		bson.M{"$project": bson.M{"_id": 1, "rating": 1, "comment": 1, "place_name": "$place_info.name", "place_id": 1}},
	}

	cursor, _ := Reviews.Aggregate(context.Background(), pipeline)
	var list []bson.M
	cursor.All(context.Background(), &list)
	if list == nil {
		list = []bson.M{}
